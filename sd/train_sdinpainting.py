"""
This giving inferior result to 'train_sdinpainting_lora.py'
"""
...
# Add the directory containing the `metrics` module to the Python path
# sys.path.append('/mnt/51eb0667-f71d-4fe0-a83e-beaff24c04fb/om/Reflection-Exploration/BrushNet')
...
def log_validation(
    ...
    for i, row in test_df.iterrows():
        ...
        images = []
        metrics = {key: [] for key in all_metrics}
        ...
        for _ in range(args.num_validation_images):
            ...
            if evaluator:
                metric = evaluator.eval_single_image(np.array(image), hdf5_path)
                draw = ImageDraw.Draw(image)
                font = ImageFont.load_default()
                text = (
                    f"PSNR: {metric['psnr']:.2f} , SSIM: {metric['ssim']:.2f}, "
                    f"LPIPS: {metric['lpips']:.2f}, FID: {metric['fid']:.2f}"
                )
                # Bolden the text
                draw.text((10, 10), text, font=ImageFont.load_default(), fill=(0, 255, 0))  # Using green color to highlight the text
                images.append(image)
                for key in metrics:
        ...
        if evaluator:
            for key in all_metrics:
                all_metrics[key].append(max(metrics[key]) if key in ["psnr", "ssim"] else min(metrics[key]))
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            ...
            table = wandb.Table(columns=["Masked Image", "Generation 1", "Generation 2", "Generation 3", "Generation 4"])
            for log in image_logs:
                row_images = []
                images = log["images"]
                validation_prompt = log["validation_prompt"]
            ...
            tracker.log({tracker_key: table})
            
            metrics_to_log = {key: np.mean(all_metrics[key]) for key in ["psnr", "fid", "ssim", "lpips"]}
            print(metrics_to_log)
            wandb.log({f"{tracker_key}_metrics": metrics_to_log}, step=step)  # Log metrics under a specific key
            # wandb.log({
            #         f"psnr": np.mean(all_metrics["psnr"]),
            #         f"fid": np.mean(all_metrics["fid"]),
            #         f"ssim": np.mean(all_metrics["ssim"]),
            #         f"lpips": np.mean(all_metrics["lpips"])
            #     }, step=step)
            # wandb.log(metrics_to_log, step=step)
            # wandb.log({f"{tracker_key}_metrics": metrics_to_log}, step=step)
        else:
...
def main(args):
    ...
    if args.report_to == "wandb":
        ...
        wandb.init(project=args.wandb_project_name, entity=os.environ["WANDB_ENTITY"])
    ...
    # Remove custom saving & loading hooks for PeftModel
    ...
    train_dataset = HDF5Dataset(
        args.train_data_dir, df, tokenizer, args.resolution
    )
